...
type WebApplication with 
  member app.UseABSample() = 
    app.MapGet("/ABs", new Func<_, _, _, _>(fun (db:BloggingContext) (skip:int) (take:int) -> 
      (AB db db.ABs).getN <@ fun _ -> true @> <@ fun e -> e.ABId @> (Ballerina.Range.WithinReason(skip, take))
      )).WithOpenApi() |> ignore
    app.MapPost("/ABs", new Func<_,_,_>(fun (db:BloggingContext) ([<FromBody>] msg: AB) -> 
    ...
    app.MapGet("/BEvents", new Func<_, _, _, _>(fun (db:BloggingContext) (skip:int) (take:int) -> 
      ...
      .WithOpenApi()  |> ignore
    app.MapPost("/ABEvents", new Func<_,_,_>(fun (db:BloggingContext) ([<FromBody>] msg: ABEvent) -> 

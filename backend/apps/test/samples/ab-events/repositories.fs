...
let rec abrepos = {|
  ef = {|
    AB = fun (db:DbContext) (table:DbSet<absample.efmodels.AB>) ->
      CrudSeq.FromDbSetToSeq table
        ({| getId = (fun id -> <@ fun e -> e.ABId = id @>); 
            setId = fun id -> fun e -> { e with ABId = id } |}) db

    ABEvent = fun (db:DbContext) (table:DbSet<absample.efmodels.ABEvent>) ->
      CrudSeq.FromDbSetToSeq table 
    ...
    AEvent = fun (db:DbContext) (table:DbSet<absample.efmodels.AEvent>) ->
      CrudSeq.FromDbSetToSeq table 
        ({| getId = (fun id -> <@ fun e -> e.ABEventId = id @>); 
            setId = fun id -> 
              fun e -> 
    ...
    BEvent = fun (db:DbContext) (table:DbSet<absample.efmodels.BEvent>) ->
      CrudSeq.FromDbSetToSeq table 
        ({| getId = (fun id -> <@ fun e -> e.ABEventId = id @>); 
            setId = fun id -> 
              fun e -> 
    ...
    ABEvent = fun (db:DbContext) (table:DbSet<absample.efmodels.ABEvent>) ->
      ...
      in {
        new Crud<absample.models.ABEvent> with
          member this.setId id = fun e -> 
            e |> ABEvent.FromUnion 
              |> ABEvent.WithRecord (fun e -> { e with ABEventId = id })
              |> ABEvent.ToUnion
          member this.getId id = <@ fun e -> (e |> ABEvent.FromUnion).ABEventId = id @>
          member this.create e = crud.create (e |> ABEvent.FromUnion)
          member this.delete id = crud.delete id
          member this.update id u = crud.update id (ABEvent.ToUnion >> u >> ABEvent.FromUnion)
          member this.get id = crud.get id |> Option.map ABEvent.ToUnion
      }
  |}
|}
...
module TransitiveExecution =
  ...
  type BusinessRuleExecutionState = { 
    ...
    static member trace rules fields = fun s -> { s with Trace = { ExecutedRules = rules; ModifiedFields=fields }::s.Trace}
    static member clear = fun s -> { s with CurrentExecutedRules = Map.empty; CurrentModifiedFields = Map.empty }
  ...
  let rec executeRulesTransitively() : State<_,BusinessRuleExecutionContext, BusinessRuleExecutionState, BusinessRuleExecutionError> = 
    ...
    state{
      ...
      // do Console.ReadLine() |> ignore
      let candidateRules = BusinessRule.getCandidateRules allBusinessRules (modifiedFields |> Map.keys |> Set.ofSeq)
            ...
            for a in businessRule.Actions do
              ...
              for modifiedFieldsByRule in modifiedFieldsByRuleVariants do
                ...
                for modifiedField in modifiedFieldsByRule |> List.ofSeq do
                  do! state.SetState(BusinessRuleExecutionState.addCurrentModifiedFields modifiedField.Key modifiedField.Value)

    ...
    let rec executeRulesTransitively
        ...
        state {
                    ...
                    | Value.ConstBool true ->
                        for a in businessRule.Actions do
                            ...
                            for modifiedFieldsByRule in modifiedFieldsByRuleVariants do
                                ...
                                do!
                                    state.SetState(
                                        BusinessRuleExecutionState.addCurrentExecutedRule
                                            businessRuleId
                                            allModifiedTargets
                                    )
                                ...
                                for modifiedField in modifiedFieldsByRule |> List.ofSeq do
                                    do!
                                        state.SetState(
                                            BusinessRuleExecutionState.addCurrentModifiedFields
                                                modifiedField.Key
                                                modifiedField.Value
                                        )
            ...
            then
                ...
                do!
                    state.SetState(
                        BusinessRuleExecutionState.updaters.AllExecutedRules(
                            BusinessRule.mergeExecutedRules executedRules'
                        )
                    )

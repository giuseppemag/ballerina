            ...
            try
                ...
                try
                    ...
                    let injectedTypes: Map<string, TypeBinding> =
                        ...
                        |> Seq.map (fun c ->
                            c.Key, (c.Key |> TypeId.Create, ExprType.RecordType Map.empty) |> TypeBinding.Create)
                        |> Map.ofSeq
                    ...
                    match
                        ((ParsedFormsContext.Parse generatedLanguageSpecificConfig jsonValue)
                            .run (codegenConfig, initialContext))
                    with
                    | Left(_, Some parsedForms) ->
                        ...
                        | Left(validatedForms, _) ->
                            ...
                            | FormsGenTarget.golang ->
                                match
                                    ParsedFormsContext.ToGolang codegenConfig parsedForms generatedPackage formName
                                with
                                | Left generatedCode ->
                                    // do Console.ReadLine() |> ignore
                                    let outputPath =
                                        System.IO.Path.Combine
                                            [| outputPath; $$"""{{inputFileNameWithoutExtension}}.gen.go""" |]
                                    ...
                                    with err ->
                                        Right(
                                            Errors.Singleton
                                                $"Error when generating output file {{err.Message.ReasonablyClamped}}"
                                        )
                                | Right err -> Right(err)
                            | _ ->
                                Right(
                                    Errors.Singleton
                                        $"Unsupported code generation target {language} when processing {inputPath}"
                                )

...
open Ballerina.DSL.FormEngine.Codegen.Golang.Generator.Model
  ...
  type GolangEntityPATCHers = { FunctionName:string; EntityNotFoundErrorConstructor:string; Writers:Map<WriterName, Writer>; CommittableWriters:List<Writer> } with
    static member ToGolang (_:GolangContext) (entities:GolangEntityPATCHers) = 
      let generatedWriters, importedWriters = entities.Writers |> Map.values |> List.ofSeq |> List.partition (fun w -> w.Kind = WriterKind.Generated)
      StringBuilder.Many(
        seq {

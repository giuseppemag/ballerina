...
open Enum
  ...
  type GolangEntityPOSTers = { FunctionName:string; EntityNotFoundErrorConstructor:string; Entities:List<{| EntityName:string; EntityType:string |}> } with
      ...
      StringBuilder.Many(
        seq {
          ...
          for e in entities.Entities do
            yield StringBuilder.Many(
                seq {
                  yield
                    StringBuilder.One(
                      $$"""deserialize{{e.EntityName}} func (id, payload) ({{e.EntityType}},error), """
                    )
          ...
          for e in entities.Entities do
            ...
            yield
              StringBuilder.One
                $$"""        var res, err = deserialize{{e.EntityName}}(entityId, entityValue);  """

        ...
        static member Validate (ctx: ParsedFormsContext) (formType: ExprType) (fr: Renderer) : Sum<ExprType, Errors> =
            ...
            sum {
                ...
                | Renderer.ListRenderer(l) ->
                    ...
                    let listRenderer =
                        ExprType.Substitute
                            (Map.empty |> Map.add { VarName = "a1" } elementRendererType)
                            genericListRenderer
                ...
                | RecordType fields ->
                    ...
                    | None ->
                        return!
                            sum.Throw(
                                Errors.Singleton(
                                    sprintf "Error: field name %A is not found in type %A" fc.FieldName formType
                                )
                            )
            ...
            state {
                ...
                | FormLauncherMode.Edit({ EntityApi = entityApi
                    ...
                    if Set.ofList [ CrudMethod.Get ] |> Set.isSuperset (configEntityApi |> snd) then
                        let! configEntityApiType =
                            ctx.TryFindType (configEntityApi |> fst).TypeId.TypeName |> state.OfSum
        ...
        static member Validate
            ...
            sum {
                ...
                | Some(id, _) when id = generatedLanguageSpecificConfig.StreamIdFieldName ->
                    ...
                    with
                    | Some(displayValue, _) when
                        displayValue = generatedLanguageSpecificConfig.StreamDisplayValueFieldName
                        ->
                        return ()
                    | _ -> return! error
                | _ -> return! error
                ...
                | missingType :: missingTypes ->
                    let missingTypeErrors =
                        ...
                        |> NonEmptyList.map (fun t ->
                            Errors.Singleton(sprintf "Error: missing type definition for %s" t.TypeName))

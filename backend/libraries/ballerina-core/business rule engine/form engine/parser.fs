...
module Parser =
    ...
    type FormLauncher with
        static member Parse
            (launcherName: string)
            (json: JsonValue)
            : State<_, CodeGenConfig, ParsedFormsContext, Errors> =
            state {
                ...
                let! (kind, formName) =
                    state.All2
                        ...
                        (JsonValue.AsString formNameJson |> state.OfSum)
                                      ...
                                      state.Any(
                                          NonEmptyList.OfList(
                                              state {
                                                  ...
                                                  let! operator =
                                                      kindJson
                                                      |> JsonValue.AsEnum BinaryOperator.AllNames
                                                      |> state.OfSum

                                                  return!
                                                      state {
                                                          let! operandsJson =
                                                              fieldsJson |> sum.TryFindField "operands" |> state.OfSum

                                                          let! (firstJson, secondJson) =
                                                              JsonValue.AsPair operandsJson |> state.OfSum
                                              ...
                                              [ state {
                                                    ...
                                                    return!
                                                        state {
                                                            let! parameterJson =
                                                                fieldsJson
                                                                |> sum.TryFindField "parameter"
                                                                |> state.OfSum

                                                            let! parameterName =
                                                                parameterJson |> JsonValue.AsString |> state.OfSum

                                                            let! bodyJson =
                                                                fieldsJson |> sum.TryFindField "body" |> state.OfSum

                                                            let! body = bodyJson |> Expr.Parse

                                                            return
                                                                Expr.Value(
                                                                    Value.Lambda({ VarName = parameterName }, body)
                                                                )
                                                        }
                                                        |> state.MapError(Errors.WithPriority ErrorPriority.High)
                                                }
                                                state {
                                                    ...
                                                    return!
                                                        state {
                                                            ...
                                                            let! operandsJson =
                                                                JsonValue.AsArray operandsJson |> state.OfSum
                                                            ...
                                                            if operandsJson.Length < 1 then
                                                                ...
                                                                let! cases =
                                                                    state.All(
                                                                        casesJson |> Seq.map (Expr.ParseMatchCase)
                                                                    )

                                                                let cases =
                                                                    cases
                                                                    |> Seq.map (fun (c, v, b) -> (c, (v, b)))
                                                                    |> Map.ofSeq
                                                ...
                                                state {
                                                    ...
                                                    return!
                                                        state {
                                                            ...
                                                            let! (firstJson, fieldNameJson) =
                                                                JsonValue.AsPair operandsJson |> state.OfSum

                                                            let! fieldName =
                                                                JsonValue.AsString fieldNameJson |> state.OfSum
                                                ...
                                                state {
                                                    ...
                                                    return!
                                                        state {
                                                            ...
                                                            let! (firstJson, caseNameJson) =
                                                                JsonValue.AsPair operandsJson |> state.OfSum

                                                            let! caseName =
                                                                JsonValue.AsString caseNameJson |> state.OfSum
                                                ...
                                                state {
                                                    ...
                                                    return!
                                                        state {
                                                            let! varNameJson =
                                                                fieldsJson |> sum.TryFindField "varName" |> state.OfSum
    ...
    type Renderer with
        static member ParseChildren
            (json: JsonValue)
            : State<RendererChildren, CodeGenConfig, ParsedFormsContext, Errors> =
            state {
                            ...
                            NonEmptyList.OfList(
                                state {
                                    let! c =
                                        ...
                                        |> Sum.fromOption (fun () ->
                                            $"Error: cannot find custom type {s}" |> Errors.Singleton)
                                        |> state.OfSum
        ...
        static member ParseUnionCase(json: JsonValue) : State<UnionCase, CodeGenConfig, ParsedFormsContext, Errors> =
            state {
                ...
                let! caseJson, fieldsJson =
                    state.All2 (args |> state.TryFindField "case") (args |> state.TryFindField "fields")
                                                ...
                                                state {
                                                    ...
                                                    return!
                                                        state {
                                                            ...
                                                            let! cases =
                                                                state.All(cases |> Seq.map (ExprType.ParseUnionCase))
    ...
    type ParsedFormsContext with
        static member ParseApis
            ...
            |> state.MapError(
                Errors.Map(
                    String.appendNewline $$"""...when parsing APIs {{apisJson.ToFSharpString.ReasonablyClamped}}"""
                )
            )

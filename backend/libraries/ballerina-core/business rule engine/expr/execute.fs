                  ...
                  for (vars, value) in values do
                      ...
                      for (_, res) in variants do
                          ...
                          | Value.Var(entityDescriptor, One entityId) ->
                              ...
                              | Value.ConstInt i ->
                                  if
                                      fieldDescriptor.Update.AsInt (One entityId) (replaceWith i) = FieldUpdateResult.ValueChanged
                                  then
                                      yield [ (fieldDescriptorId, Multiple(Set.singleton entityId)) ] |> Map.ofList
                              | Value.ConstGuid id ->
                                  if
                                      fieldDescriptor.Update.AsRef (One entityId) (replaceWith id) = FieldUpdateResult.ValueChanged
                                  then
                                      yield [ (fieldDescriptorId, Multiple(Set.singleton entityId)) ] |> Map.ofList
                              | _ -> ()
                          | _ -> () ]

...
module Errors =
  ...
  type Error = { Message:string; Priority:ErrorPriority } with
    static member Updaters = {|
      Message = fun u err -> { err with Message=u(err.Message)}
      Priority = fun u err -> { err with Priority=u(err.Priority)}
    |}
  type Errors = { Errors:NonEmptyList<Error> } with
    static member Singleton e  = { Errors=NonEmptyList.One({ Message=e; Priority=ErrorPriority.Low }) }
    static member Concat(e1,e2)  = { Errors=NonEmptyList.OfList(e1.Errors.Head, e1.Errors.Tail @ (e2.Errors |> NonEmptyList.ToList)) }
    static member Map f e = { e with Errors=e.Errors |> NonEmptyList.map(Error.Updaters.Message f)  }
    static member WithPriority p e = 
      { e with Errors=e.Errors |> NonEmptyList.map(Error.Updaters.Priority(replaceWith p))  }
    static member HighestPriority e =   
      ...
      | x::xs -> 
        { e with Errors=NonEmptyList.OfList(x,xs)}
      | []-> 

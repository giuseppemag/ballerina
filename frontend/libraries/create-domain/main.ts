...
const optionDefinitions = [
  {
    ...
    alias: "n",
    type: String,
    helpText: `The name of the domain folder, also used for type definitions/repository/etc.`,
    required: true,
  },
  {
    name: "path",
    type: String,
    helpText: `The path of the domain, relative to the location from which the tool is executed`,
    required: true,
  },
  // { name: 'frameworkPath', type: String, helpText: `The path of the "core" folder, relative to the location from which the tool is executed`, required: false },
  {
    name: "override",
    type: Boolean,
    helpText: `[DANGEROUS] Overwrite existing files`,
    required: false,
  },
  {
    name: "help",
    type: Boolean,
    helpText: `Print this guide`,
    required: false,
  },
];
...
const showHelp = () => {
  const sections = [
    ...
    {
      header: "Options",
      optionList: optionDefinitions.map((_) => ({
        name: _.name,
        typeLabel:
          _.type == String ? "string" : _.type == Boolean ? "boolean" : "",
...
try {
  ...
  if (!!options.help) {
    ...
    const domainNameCamelCased = domainNameHyphenated.replace(
      /-([a-z])/g,
      function (g) {
    ...
    const createFile = (f: string, defaultText: string) => {
      ...
      if (override && fs.existsSync(p)) fs.rmSync(p);
      fs.appendFileSync(p, defaultText);
    };

    const stateDefault =
      () => `import { ForeignMutationsInput, simpleUpdater, Unit } from "ballerina-core"
...
export const Co = CoTypedFactory<${capitalize(domainNameCamelCased, true)}ReadonlyContext, ${capitalize(domainNameCamelCased, true)}WritableState>()`;
    ...
    const runnerDefault =
      () => `import { ${capitalize(domainNameCamelCased, true)}ForeignMutationsExpected } from "../state";

...
export const EnumMultiselectForm = <Context extends FormLabel & BaseEnumContext<Element>, ForeignMutationsExpected, Element extends EnumReference>(
  ...
  return Template.Default<Context & Value<OrderedMap<Guid, Element>> & { disabled: boolean }, EnumFormState<Context, Element>, ForeignMutationsExpected & { onChange: OnChange<OrderedMap<Guid, Element>>; }, EnumMultiselectView<Context, Element, ForeignMutationsExpected>>(props => <>
    <props.view {...props}
      context={{
        ...props.context,
        selectedIds: props.context.value.map(_ => _.Value).valueSeq().toArray(),
          ...
          const newSelection = _
            .flatMap(_ => { 
              ...
              if (selectedItem != undefined) {
                const item: [string, Element] = [selectedItem.Value, selectedItem];
                return [item];
              }
              return [];
            });

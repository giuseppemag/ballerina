    ...
    Object.assign(
      ...
      > => {
        ...
        return Co.GetState().then((current) => {
          ...
          return (
            synchronizers[k][mutation](mutationArg as any) as Coroutine<
              Context & CollectionEntity<Collections[k]>,
              CollectionEntity<Collections[k]>,
              SynchronizationResult
            >
            ...
            .embed<Context & SynchronizedEntities, SynchronizedEntities>(
              (_) => {
                ...
                if (AsyncState.Operations.hasValue(entities.sync)) {
                  ...
                  if (entity != undefined) return { ..._, ...entity };
                }
                return undefined;
              },
              Fun(
                Collection<Collections[k]>().Updaters.Core.entity.set(entityId),
              ).then(widening_k),
            )
            .then((syncResult) =>
      ...
      },
      {
            ...
            .then(() =>
              synchronizers[k]
                ...
                .embed<Context & SynchronizedEntities, SynchronizedEntities>(
                  ...
                  Fun(
                    Collection<Collections[k]>().Updaters.Core.entity.set(
                      entityId,
                    ),
                  ).then(widening_k),
                )
        ...
        > => {
          ...
          return CoColl.SetState(
            // Collection<Collections[k]>().Updaters.Core.entity.remove(entityId)
            Collection<Collections[k]>().Updaters.Core.removed(
              entityId,
              replaceWith(true),
            ),
          )
            ...
            .then((_) =>
              Co.GetState().then((current) => {
                ...
                if (!entity || entity.value.sync.kind == "error")
                  return Co.Return<SynchronizationResult>("completed");
                return Co.All<SynchronizationResult>(dependees).then(
                  (syncResults) =>
                    Co.Return(
...
export const collectionCheckNotDirty =
  <Collections, CollectionMutations, WholeCollectionMutations>() =>
  <k extends keyof Collections & keyof CollectionMutations>([id, e]: [
    ...
    return (
      ...
      e.entities.sync.value.get(id)?.value.dirty != "not dirty"
    );
  };
...
export const collectionEntityUpdater =
  <
    Collections,
    CollectionMutations,
    WholeCollectionMutations,
...
export const withTrivialComparator = <f, ma>(
  _: f,
): [f, MutationArgumentComparator<ma>] => [_, (_) => "=="];
...
export const collectionSynchronizationContext = <
  Context,
  Collections,
  CollectionMutations,
  WholeCollectionMutations,
...
>(
  ...
  let synchronizers: CollectionSynchronizers<
    Context,
    Collections,
    CollectionMutations,
    WholeCollectionMutations
  ...
  Object.keys(entityDescriptors).forEach((k_s) => {
    const k = k_s as keyof Collections &
      keyof CollectionMutations &
      keyof WholeCollectionMutations;
      ...
      if (
        ...
        (synchronizers[k] as any)[field] = (mutationArg: any) => {
          return insideEntitySynchronizedAndDebounced(
            (entityDescriptors[k] as any)[field](mutationArg),
          ) as any;
        };
    });

    (synchronizers[k] as any)["add"] = (
      entity: CollectionEntity<any>,
      position?: InsertionPosition,
    ) =>
      insideEntitySynchronizedAndDebounced(
        (entityDescriptors[k] as any)["add"](entity, position),
      ) as any;
    (synchronizers[k] as any)["remove"] = (entityId: Guid) =>
      insideEntitySynchronizedAndDebounced(
        (entityDescriptors[k] as any)["remove"](entityId),
      ) as any;
    (synchronizers[k] as any)["wholeMutations"] = {} as any;
    Object.keys(entityDescriptors[k]["wholeMutations"]).forEach((field) => {
      (synchronizers[k] as any)["wholeMutations"][field] = (mutationArg: any) =>
        (entityDescriptors[k]["wholeMutations"] as any)[field](mutationArg);
    });
  });
  const loaders: CollectionLoaders<
    Context,
    Collections,
    CollectionMutations,
    WholeCollectionMutations,
  ...
  Object.keys(entityDescriptors).forEach((k_s) => {
    const k = k_s as keyof Collections &
      keyof CollectionMutations &
      keyof WholeCollectionMutations;
    loaders[k] = collectionEntityLoader<
      Context,
      Collections,
      CollectionMutations,
      WholeCollectionMutations,
  ...
  const dirtyCheckers: CollectionDirtyCheckers<
    Collections,
    CollectionMutations,
    WholeCollectionMutations
  > = {} as any;
  ...
  const dirtySetters: CollectionDirtySetters<
    Context,
    Collections,
    CollectionMutations,
    WholeCollectionMutations,
  ...
  const updaters: CollectionUpdaters<
    Collections,
    CollectionMutations,
    WholeCollectionMutations,
    SynchronizedEntities
  ...
  Object.keys(entityDescriptors).forEach((k_s) => {
    const k = k_s as keyof Collections &
      keyof CollectionMutations &
      keyof WholeCollectionMutations;
    updaters[k] = collectionEntityUpdater<
      Collections,
      CollectionMutations,
      WholeCollectionMutations,
      SynchronizedEntities
  ...
  const mutationComparators: CollectionMutationArgumentComparators<
    Collections,
    CollectionMutations
  > = {} as any;
  Object.keys(entityDescriptors).forEach((k_s) => {
    ...
    Object.keys(entityDescriptors[k]).forEach((field) => {
      if (
        ...
        (mutationComparators[k] as any)[field] = (entityDescriptors[k] as any)[
          field
        ][1];
    });
  });
  const wholeMutationComparators: WholeCollectionMutationArgumentComparators<
    Collections,
    WholeCollectionMutations
  > = {} as any;
  Object.keys(entityDescriptors).forEach((k_s) => {
    ...
    Object.keys(entityDescriptors[k]).forEach((field) => {
      if (
        ...
        (wholeMutationComparators[k] as any)[field] = (
          entityDescriptors[k] as any
        )[field][1];
    });
  });
  return [

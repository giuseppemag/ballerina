...
export const insideEntitySynchronizedAndDebounced = <Context, E>(
  ...
  return Co.SetState(
    Entity<E>().Updaters.Core.value(
      Debounced.Updaters.Core.dirty(
        replaceWith<DirtyStatus>("dirty but being processed"),
      ),
    ),
  ).then(() =>
    k
      .embed<Context & Entity<E>, Entity<E>>(
        (_) => ({ ..._, ..._.value }),
        (u) =>
          Entity<E>().Updaters.Core.value(Debounced.Updaters.Core.value(u)),
      )
...
export const singletonEntityLoader =
  <Context, Singletons, SingletonMutations, SynchronizedEntities>(
  ...
  > => {
    ...
    return (
      synchronizers[k][mutation](mutationArg) as Coroutine<
        Context & Entity<Singletons[k]>,
        Entity<Singletons[k]>,
        SynchronizationResult
      >
      ...
      .embed<Context & SynchronizedEntities, SynchronizedEntities>(
        (_) => ({ ..._, ...narrowing_k(_).entity }),
        Singleton<Singletons[k]>().Updaters.Core.entity.then(widening_k),
      )
      .then((syncResult) =>
...
export const singletonDirtySetter =
  <Context, Singletons, SingletonMutations, SynchronizedEntities>() =>
  <k extends keyof Singletons & keyof SingletonMutations>(
    k: k,
    narrowing_k: BasicFun<SynchronizedEntities, Singleton<Singletons[k]>>,
...
export const singletonCheckNotDirty =
  <Singletons, SingletonMutations>() =>
  <k extends keyof Singletons & keyof SingletonMutations>([_, e]: [
    Unit,
    Singleton<Singletons[k]>,
  ]): boolean =>
    e.entity.value.dirty != "not dirty";
...
export const singletonEntityUpdater =
  <Singletons, SingletonMutations, SynchronizedEntities>() =>
  <k extends keyof Singletons & keyof SingletonMutations>(
    widening_k: BasicFun<
      BasicUpdater<Singleton<Singletons[k]>>,
      Updater<SynchronizedEntities>
    ...
    Fun(([_, u]) =>
      widening_k(Singleton<Singletons[k]>().Updaters.Template.entityValue(u)),
    );
...
export const singletonSynchronizationContext = <
  Context,
  Singletons,
  SingletonMutations,
  SynchronizedEntities,
>(
  ...
  const synchronizers: SingletonSynchronizers<
    Context,
    Singletons,
    SingletonMutations
  > = {} as any;
  ...
  const loaders: SingletonLoaders<
    Context,
    Singletons,
    SingletonMutations,
    SynchronizedEntities
  ...
  Object.keys(entityDescriptors).forEach((k_s) => {
    const k = k_s as keyof Singletons & keyof SingletonMutations;
    loaders[k] = singletonEntityLoader<
      Context,
      Singletons,
      SingletonMutations,
      SynchronizedEntities
  ...
  const dirtyCheckers: SingletonDirtyCheckers<Singletons, SingletonMutations> =
    {} as any;
  Object.keys(entityDescriptors).forEach((k_s) => {
    const k = k_s as keyof Singletons & keyof SingletonMutations;
    dirtyCheckers[k] = Fun(
      singletonCheckNotDirty<Singletons, SingletonMutations>(),
    );
  });
  const dirtySetters: SingletonDirtySetters<
    Context,
    Singletons,
    SingletonMutations,
    SynchronizedEntities
  ...
  Object.keys(entityDescriptors).forEach((k_s) => {
    const k = k_s as keyof Singletons & keyof SingletonMutations;
    dirtySetters[k] = singletonDirtySetter<
      Context,
      Singletons,
      SingletonMutations,
      SynchronizedEntities
  ...
  const updaters: SingletonUpdaters<
    Singletons,
    SingletonMutations,
    SynchronizedEntities
  > = {} as any;
  Object.keys(entityDescriptors).forEach((k_s) => {
    const k = k_s as keyof Singletons & keyof SingletonMutations;
    updaters[k] = singletonEntityUpdater<
      Singletons,
      SingletonMutations,
      SynchronizedEntities
    >()(entityDescriptors[k].widening);
  });
  const mutationComparators: SingletonMutationArgumentComparators<
    Singletons,
    SingletonMutations
  > = {} as any;
  Object.keys(entityDescriptors).forEach((k_s) => {

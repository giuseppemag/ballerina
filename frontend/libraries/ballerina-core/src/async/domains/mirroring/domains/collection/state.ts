...
export const Collection = <E>() => {
  return {
    Default: (
      entities: OrderedMap<Guid, CollectionEntity<E>>,
    ): Collection<E> => ({
      entities: Synchronized.Default(
        unit,
        AsyncState.Default.loaded(
          entities.map((_) => ({ ..._, removed: false })),
        ),
      ),
    }),
    Updaters: {
      Core: {
        ...
        entity: {
          ...
          remove: (id: Guid): Updater<Collection<E>> =>
            Collection<E>().Updaters.Core.entities(
              Synchronized.Updaters.sync(
                AsyncState.Operations.map(OrderedMapRepo.Updaters.remove(id)),
              ),
            ),
          set:
            (id: Guid) =>
        ...
        removed: (id: Guid, u: BasicUpdater<boolean>): Updater<Collection<E>> =>
          Collection<E>().Updaters.Core.entity.set(id)(
            CollectionEntity<E>().Updaters.Core.removed(u),
          ),

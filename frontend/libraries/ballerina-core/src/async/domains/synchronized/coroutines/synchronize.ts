...
export const Synchronize = <value, syncResult, context = Unit>(
	...
	return Co.SetState(Synchronized.Updaters.sync(AsyncState.Updaters.toReloading())).then(() => 
			Co.GetState().then(current => Co.Await(() => p(current as value & context), id).then(apiResult => {
		if (apiResult.kind == "l") {
			...
			return Co.Wait(delayBetweenAttemptsInMs).then(() => 
				Synchronize<value, syncResult, context>(p, errorProcessor, maxAttempts - 1, delayBetweenAttemptsInMs));
		} else {
...
export const SynchronizeWithValueUpdater = <value, syncResult, context = Unit>(
	...
	return Co.SetState(Synchronized.Updaters.sync(AsyncState.Updaters.toReloading())).then(() => 
		...
		if (apiResult.kind == "l") {
			...
			return Co.Wait(delayBetweenAttemptsInMs).then(() => 
				SynchronizeWithValueUpdater<value, syncResult, context>(p, errorProcessor, maxAttempts - 1, delayBetweenAttemptsInMs));
		} else {
			return Co.SetState(Synchronized.Updaters.sync(AsyncState.Updaters.toError(apiResult.value))).then(() => 
				Co.Return<ApiResultStatus>("permanent failure")
			);

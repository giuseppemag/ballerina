...
export const StreamDataLoader = <
  Element extends { Id: string },
  foreignMutations,
>() => {
  const operations = InfiniteStreamState<Element>().Operations;
  const LoaderTemplate = StreamCo<Element>().Template<foreignMutations>(
    Loader<Element>(),
    {
  ...
  return LoaderTemplate.mapContext<
    InfiniteStreamReadonlyState & InfiniteStreamWritableState<Element>
  >((_) => _);
};

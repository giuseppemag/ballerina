...
export const simpleUpdater =
  ...
  <Field extends keyof Entity>(field: Field): SimpleUpdater<Entity, Field> =>
    ({
      [field]: Fun(
        (fieldUpdater: BasicUpdater<Entity[Field]>): Updater<Entity> => {
...
export type WidenedChildren<p, children> = {
  ...
  : Unit) &
  (children extends { Template: Unit }
    ? {
        Template: WidenedChildren<p, children["Template"]>;
      }
    : Unit) &
  (children extends { Coroutine: Unit }
    ? {
...
export const simpleUpdaterWithChildren =
  <Entity>() =>
    ...
    ({
      [field]: Object.assign(
        Fun((fieldUpdater: BasicUpdater<Entity[Field]>): Updater<Entity> => {
          const basicUpdater: BasicUpdater<Entity> = (currentEntity) => ({
            ...currentEntity,
            [field]: fieldUpdater(currentEntity[field]),
          });
        ...
        {
          children: widenChildren<Entity, Entity[Field], children>(
            simpleUpdater<Entity>()(field)[field],
            children,
          ),
        },
      ),
...
export const widenChildren = <p, child, children>(
  ...
  Object.keys(children as any).forEach((key) => {
    ...
    if (Object.keys(fieldOfChildUpdater).includes("children")) {
      (result as any)[key]["children"] = widenChildren<p, any, any>(
        childToParent,
  ...
  const updateBlock = (block: string) => {
    if ((children as any)[block] != undefined) {
      (result as any)[block] = widenChildren<p, any, any>(
        childToParent,
        (children as any)[block],
      );
    }
  };
  updateBlock("Core");

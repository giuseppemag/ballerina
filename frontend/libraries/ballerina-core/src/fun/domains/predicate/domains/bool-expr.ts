...
export type LeafPredicatesEvaluators<Leaf, Context> = {
  [leaf in keyof Leaf]: BasicFun<Leaf[leaf], Predicate<Context>>;
};
export type BoolExpr<Leaf> = (
  ...
  eval: <Context>(
    evalLeaf: LeafPredicatesEvaluators<Leaf, Context>,
  ) => Predicate<Context>;
};

export const BoolExpr = {
  Default: Object.assign(
    <Leaf>(_: BoolExprSerializable<Leaf>): BoolExpr<Leaf> => {
      return Object.assign(
        typeof _ == "boolean"
          ...
          : _.kind == "and" || _.kind == "or"
            ? {
                kind: _.kind,
                operands: [
                  BoolExpr.Default(_.operands[0]),
                  BoolExpr.Default(_.operands[1]),
                ] as [BoolExpr<Leaf>, BoolExpr<Leaf>],
              }
            : _.kind == "not"
                  ...
                  : { kind: _.kind },
        {
          ...
          eval: function <Context>(
            ...
            return this.kind == "not"
              ? this.operand.eval(evalLeaf).not()
              : this.kind == "and"
                ? this.operands[0]
                    .eval(evalLeaf)
                    .and(this.operands[1].eval(evalLeaf))
                : this.kind == "or"
                  ? this.operands[0]
                      .eval(evalLeaf)
                      .or(this.operands[1].eval(evalLeaf))
                  : this.kind == "leaf"
                    ? evalLeaf[this.operation](this.arguments)
                    : this.kind == "true"
                      ? Predicate.True()
                      : Predicate.False();
          },
    ...
    {
      true: <Leaf>(): BoolExpr<Leaf> =>
        BoolExpr.Default({
          kind: "true",
        }),
      false: <Leaf>(): BoolExpr<Leaf> =>
        BoolExpr.Default({
          kind: "false",
        }),
      leaf:
        <Leaf>() =>
        <operation extends keyof Leaf>(
          operation: operation,
          args: Leaf[operation],
        ): BoolExpr<Leaf> =>
          BoolExpr.Default({
